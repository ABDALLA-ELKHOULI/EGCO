...
def _strict_samples():
    """هل يُفشل غياب العيّنة بدل أن يتخطّاها؟

    كان هذا معلّقاً على `CI` الذي تضبطه GitHub تلقائياً — فحاصر كل شحن: المستودع
    **عام**، والعيّنات كشوف حسابات حقيقية بأسماء موردين ومقاولين ومبالغهم، فلا
    يجوز رفعها. النتيجة كانت بوابةً لا تُعبَر أبداً، وهي ليست بوابةً بل سدّ.

    الصرامة الآن **اختيار صريح** (`EGCO_REQUIRE_SAMPLES=1`): تُضبط حيث تتوفّر
    العيّنات فعلاً — جهاز المستخدم، أو CI بعد تجهيز العيّنات مشفّرة. وحيث لا
    تتوفّر يبقى التخطّي، **لكن `-rs` يطبع كل سبب** ويُظهر الخطوة عدد المتخطّى،
    فلا يعود «أخضر» يعني «اختُبر كل شيء».

    والتشغيل المرجعي يبقى محلياً قبل كل وسم — هناك تُشغَّل السويّة كاملة بلا
    تخطٍّ واحد. CI يبني الحزم؛ الاختبار الحقيقي يقع قبله.
    """
    return bool(os.environ.get('EGCO_REQUIRE_SAMPLES'))


def _in_ci():
    return _strict_samples()
...
def sample_missing(*paths, kind='exists'):
    ...
    if _in_ci():
        pytest.fail(
            'عيّنة ناقصة و EGCO_REQUIRE_SAMPLES مضبوط — فشل صريح بدل تخطٍّ صامت: '
